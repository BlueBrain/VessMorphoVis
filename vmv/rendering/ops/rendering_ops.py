...
def render_scene_at_resolution(file_name='image',
    ...
    scene_bounding_box = vmv.bbox.compute_scene_bounding_box()
    ...
    if view == 'FRONT':
        ...
        camera_object = vmv.rendering.camera.create_default_camera(location=camera_location_z)

        # Rotate the camera
        vmv.camera.ops.rotate_camera_for_front_view(camera_object=camera_object)

    # Side view
    elif view == 'SIDE':

        # Add a camera along the x-axis
        camera_object = vmv.rendering.camera.create_default_camera(location=camera_location_x)

        # Rotate the camera
        vmv.camera.ops.rotate_camera_for_side_view(camera_object=camera_object)

    # Top view
    elif view == 'TOP':

        # Add a camera along the y-axis
        camera_object = vmv.camera.create_default_camera(location=camera_location_y)

        # Rotate the camera
        vmv.camera.ops.rotate_camera_for_top_view(camera_object=camera_object)

    # Update the camera resolution
    vmv.camera.ops.set_camera_resolution_for_specific_view(
        camera_object, film_base_resolution, view)

    # Activate the selected camera for rendering
    vmv.camera.ops.set_active_camera(camera_object)
...
def render_scene_to_scale(file_name='image',
    ...
    if view == 'FRONT':
        ...
        # Add a camera along the z-axis
        camera_object = vmv.camera.create_default_camera(location=camera_location_z)
    ...
    # Side view
        ...
        # Add a camera along the x-axis
        camera_object = vmv.camera.create_default_camera(location=camera_location_x)
    ...
    # Update the camera resolution
    vmv.camera.ops.set_camera_resolution_to_scale_for_specific_view(
...
def render_full_view(view_bounding_box,
    ...
    vmv.scene.ops.delete_list_objects([camera])
