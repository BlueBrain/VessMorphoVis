...
def import_obj_file(input_directory,
    ...
    if not os.path.isfile(file_path):
        vmv.logger.log('LOADING ERROR: cannot load [%s]' % file_path)

    # Deselect all the objects in the scene
    vmv.scene.ops.deselect_all()

    vmv.logger.log('Loading [%s]' % file_path)
...
def import_object_from_blend_file(input_directory,
    ...
    vmv.logger.log('Importing [%s]' % file_path)
    ...
    for i_object in data_dst.objects:
        if i_object is not None:
            current_scene.collection.objects.link(i_object)
