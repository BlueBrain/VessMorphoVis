...
def extend_bounding_boxes(bounding_boxes_list):
    ...
    bounding_box = vmv.bbox.BoundingBox(p_min, p_max)
...
def get_object_bounding_box(scene_object):
    ...
    # Build bounding box object
    bounding_box = vmv.bbox.BoundingBox(center=center, bounds=bounds)
...
def confirm_object_bounding_box(scene_object):
    ...
    # Build bounding box object
    bounding_box = vmv.bbox.BoundingBox(p_min=p_min, p_max=p_max)
...
####################################################################################################
# @compute_bounding_box_for_list_of_points
####################################################################################################
def compute_bounding_box_for_list_of_points(points):
    """Computes the bounding box of a list of points.

    :param points:
        A list of points to compute the bounding box to.
    :return:
        The bounding box of the given list of points
    """
    ...
    # Initialize pMax
    p_max = Vector((-vmv.consts.Math.INFINITY,
                    -vmv.consts.Math.INFINITY,
                    -vmv.consts.Math.INFINITY))

    # Iterate over the points and update p_min and p_max to construct the bounding box
    for point in points:
        if point[0] < p_min[0]:
            p_min[0] = point[0]
        if point[1] < p_min[1]:
    ...
    # Return a reference to the computed bounding box
    return bounding_box
...
def compute_unified_extent_bounding_box(extent):
    ...
    unified_bounding_box = vmv.bbox.BoundingBox(p_min=p_min, p_max=p_max)
...
def compute_unified_bounding_box(non_unified_bounding_box):
    ...
    unified_bounding_box = vmv.bbox.BoundingBox(center=non_unified_bounding_box.center,
...
def draw_bounding_box(bbox,
    ...
    for i, line_coordinates in enumerate(point_list):
        ...
        line = vmv.geometry.ops.draw_line(
    ...
    for i in range(len(line_list)):
        line_list[i] = vmv.scene.ops.convert_object_to_mesh(line_list[i])

    # Union them into a single mesh object
    bbox_mesh = vmv.mesh.ops.join_mesh_objects(line_list, name=name)
