...
# @triangulate_faces
####################################################################################################
def triangulate_faces(bmesh_object):
...
def get_nearest_face_index(bmesh_object,
    ...
    shortest_distance = vmv.consts.Math.INFINITY
...
def rotate_face_in_object_towards_point(bmesh_object,
    ...
    face = vmv.bmeshi.ops.get_face_from_index(bmesh_object, face_index)
...
def extrude_point_to_point(bmesh_object,
    ...
    # Extrude the face
    face = vmv.bmeshi.extrude_face_to_face(bmesh_object, face)
...
def get_face_shortest_edge(bmesh_object,
    ...
    shortest_edge_length = vmv.consts.Math.INFINITY
...
def get_face_longest_edge(bmesh_object,
    ...
    longest_edge_length = -1 * vmv.consts.Math.INFINITY
...
def get_indices_of_faces_intersecting_sphere(bmesh_object,
    ...
    for face in bmesh_object.faces[:]:
        ...
        if vertex_intersection_exists is False:
            for edge in face.edges[:]:
                if vmv.geometry.ops.sphere_line(sphere_center, sphere_radius,
